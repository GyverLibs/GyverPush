...
## Использование
### Как это работает
- Что мы хотим: получать от ESP8266/ESP32 пуш-уведомление на смартфон или ПК
- **Браузер** ПК или смартфона (далее - клиент) подписывается на уведомления при помощи нативного механизма, в результате получается "токен"
- Используя токен, можно отправить клиенту уведомление. Это непростой процесс, использующий специальное шифрование, поэтому сама ESP-шка с ним не справится
- Предлагается использовать промежуточный PHP сервер, который принимает токен от есп и отправляет с его помощью пуш. Вариант не самый безопасный, но очень простой и рабочий, не требующий ведения базы данных подписок на сервере

"Из коробки" доступен мой сайт https://push.gyver.ru/ - он как даёт браузеру подписку, так и обрабатывает запросы от ESP. Т.е. уведомления будут приходить от лица этого сайта, а библиотека будет обращаться к этому же серверу для отправки пуша. В репозитории прилагаю исходник веб-приложения и php скрипт - можно поднять аналогичный push.gyver.ru сервис буквально в пару кликов на своём сервере и сайте.
...
### Получение токена
Заходим на https://push.gyver.ru/, жмём подписаться, разрешаем уведомления. Будет получен токен - он уникален для каждого устройства и браузера. Токен довольно тяжёлый - около 500 байт, поэтому хранить его лучше в PROGMEM (статично) или файле (можно менять в процессе работы). На сайте два окошка с токеном - один "сырой", второй оформлен для PROGMEM.
...
### Свой сервер
- Установить на php-сервер библиотеку `web-push-bundle`: https://packagist.org/packages/minishlink/web-push-bundle
...
- В GyverPush указать свой сервер, порт и путь к `push.php`

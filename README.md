...
### Свой сервер
- Понадобится домен с поддержкой SSL (например Let's Encrypt). Задача домена - только отобразить страничку сайта, поэтому он может быть на другом сервере или вообще на GitHub Pages
